...
const portfolioProcedureColumns = `id::text, procedure_code, version_no, title, source_ref, lifecycle_status, coalesce(to_char(effective_from,'YYYY-MM-DD'),''), coalesce(to_char(effective_to,'YYYY-MM-DD'),''), calendar_rules, access_rules, accepted_formats, retention_rules, transfer_rules, coalesce(to_char(approved_at at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),''), coalesce(approved_by_user_id::text,''), coalesce(to_char(published_at at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),''), coalesce(published_by_user_id::text,''), created_by_user_id::text, to_char(created_at at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'), to_char(updated_at at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'), coalesce(to_char(superseded_at at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),''), coalesce(to_char(withdrawn_at at time zone 'UTC','YYYY-MM-DD"T"HH24:MI:SS.US"Z"'),''), institution_id`
