-- Signed evidence must be bound to a ready eArhiva version. The database, not
-- the browser, copies immutable source provenance into the legal ledger.
alter table education_signed_artifact_evidence
	drop constraint if exists education_signed_artifact_evidence_storage_required,
	drop constraint if exists education_signed_artifact_evidence_storage_provenance_required;

alter table education_signed_artifact_evidence
	add constraint education_signed_artifact_evidence_storage_required
		check (storage_document_id is not null and storage_version_id is not null) not valid,
	add constraint education_signed_artifact_evidence_storage_provenance_required
		check (document_sha256 ~ '^[0-9a-f]{64}$' and btrim(storage_bucket) <> '' and btrim(storage_object_key) <> '') not valid;
...
begin
	...
	case new.artifact_type
		when 'decision' then
			select exists(select 1 from education_decisions where id=new.artifact_id and institution_id=new.institution_id and status <> 'blocked') into artifact_found;
		when 'publication' then
			select exists(select 1 from education_publications where id=new.artifact_id and institution_id=new.institution_id and publication_status <> 'retras') into artifact_found;
		when 'managerial_document' then
			select exists(select 1 from education_managerial_documents where id=new.artifact_id and institution_id=new.institution_id and document_status <> 'archived') into artifact_found;
		when 'meeting_document' then
			select exists(select 1 from education_meeting_documents where id=new.artifact_id and institution_id=new.institution_id) into artifact_found;
		when 'meeting_minute' then
			select exists(select 1 from education_meeting_minutes where id=new.artifact_id and institution_id=new.institution_id) into artifact_found;
		when 'meeting_resolution' then
			select exists(select 1 from education_meeting_resolutions where id=new.artifact_id and institution_id=new.institution_id) into artifact_found;
	end case;
	...
	select version.source_sha256, version.source_bucket, version.source_object_key
	...
	where document.id=new.storage_document_id and version.id=new.storage_version_id
		and document.institution_id=new.institution_id and version.institution_id=new.institution_id
