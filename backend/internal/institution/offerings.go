...
func (s *Service) CreateOfferingAuthorization(w http.ResponseWriter, r *http.Request) {
	...
	if !replay {
		...
		var sourceID string
		now := time.Now().UTC()
		err = tx.QueryRow(r.Context(), `insert into school_regulatory_sources(tenant_code,institution_id,source_kind,citation,article_reference,issuer,source_url,published_on,consolidated_on,effective_from,effective_to,checksum_sha256,status,verified_at,verified_by_subject,revalidation_owner_subject,created_by_subject,updated_by_subject)
			values($1,$2,$3,$4,$5,$6,$7,nullif($8,'')::date,nullif($9,'')::date,$10::date,nullif($11,'')::date,$12,'active',$13,$14,$14,$14,$14) returning id::text`, tenantCode, institutionID, input.Source.SourceKind, strings.TrimSpace(input.Source.Citation), strings.TrimSpace(input.Source.ArticleReference), strings.TrimSpace(input.Source.Issuer), strings.TrimSpace(input.Source.SourceURL), optionalString(input.Source.PublishedOn), optionalString(input.Source.ConsolidatedOn), input.EffectiveFrom, optionalString(input.EffectiveTo), strings.TrimSpace(input.Source.ChecksumSHA256), now, actor).Scan(&sourceID)
		if err == nil {
