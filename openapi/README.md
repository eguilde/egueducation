...
`openapi.json` is the generated, versioned OpenAPI 3.1.1 contract. It covers every concrete `GET`, `POST`, `PUT`, `PATCH`, and `DELETE` route registered in `backend/cmd/server/main.go` (currently 453 API operations), plus the public OIDC-provider routes.
