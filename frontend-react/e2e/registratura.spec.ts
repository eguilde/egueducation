...
async function authenticatedRegistratura(page: Page, activeDocument = document) {
  ...
  await page.route('**/api/registratura/documents?**', (route) => route.fulfill({ contentType: 'application/json', body: JSON.stringify({ items: [activeDocument], total: 45, page: Number(new URL(route.request().url()).searchParams.get('page') ?? 1), pageSize: 20 }) }));
  await page.route('**/api/registratura/parties?**', (route) => route.fulfill({ contentType: 'application/json', body: JSON.stringify({ items: [{ id: 'party-school', party_type: 'institution', display_name: 'Școala Test', is_default_organization: true }, { id: 'party-1', party_type: 'physical', display_name: 'Ana Pop' }], total: 2, page: 1, pageSize: 50 }) }));
  ...
  await page.route(`**/api/registratura/documents/${activeDocument.id}`, (route) => route.fulfill({ contentType: 'application/json', body: JSON.stringify(activeDocument) }));
  await page.route(`**/api/registratura/documents/${activeDocument.id}/versions`, (route) => route.fulfill({ contentType: 'application/json', body: '[]' }));
  await page.route(`**/api/registratura/documents/${activeDocument.id}/attachments`, (route) => route.fulfill({ contentType: 'application/json', body: '[]' }));
  await page.route(`**/api/registratura/documents/${activeDocument.id}/workflow-history`, (route) => route.fulfill({ contentType: 'application/json', body: '[]' }));
...
test('registratura creates an incoming document with operational fields', async ({ page }) => {
  ...
  await expect(createDialog.getByRole('textbox', { name: 'Destinatar', exact: true })).toHaveValue('Școala Test');
  ...
  await createDialog.getByLabel('Termen document').fill('2026-09-30');
  ...
  const createRequest = page.waitForRequest((request) => request.url().endsWith('/api/registratura/documents') && request.method() === 'POST');
  await createDialog.getByRole('button', { name: 'Salvează' }).click();
  expect((await createRequest).postDataJSON()).toMatchObject({ subject: 'Cerere înscriere', correspondent: 'Ana Pop', assigned_to: 'Școala Test', external_number: 'EXT-11', due_date: '2026-09-30', department_ids: ['dept-1'] });
...
test('registratura mirrors the Costești table controls with server-side requests', async ({ page }) => {
  ...
  await expect(page.getByText('Înregistrat')).toBeVisible();
  await expect(page.getByText('16.08.2026')).toBeVisible();
  ...
  const filtered = page.waitForRequest((request) => new URL(request.url()).searchParams.get('filter.registry_number') === 'REG-1');
  await page.getByLabel('Filtru coloană Nr. Doc').fill('REG-1');
  await page.getByLabel('Filtru coloană Nr. Doc').press('Enter');
  expect(new URL((await filtered).url()).searchParams.get('page')).toBe('1');
...
test('registratura hides mutation and workflow actions for a cancelled document while retaining history and PDF', async ({ page }) => {
  const cancelled = { ...document, id: 'doc-cancelled', registry_number: 'REG-CANCELLED', subject: 'Document anulat', status: 'ANULAT', cancellation_reason: 'Înregistrare duplicată' };
  await authenticatedRegistratura(page, cancelled);
  await expect(page.getByRole('button', { name: 'Istoric REG-CANCELLED' })).toBeVisible();
  await expect(page.getByRole('button', { name: 'PDF REG-CANCELLED' })).toBeVisible();
  await expect(page.getByRole('button', { name: 'Editează REG-CANCELLED' })).toHaveCount(0);
  await expect(page.getByRole('button', { name: 'Anulează REG-CANCELLED' })).toHaveCount(0);
  await expect(page.getByRole('button', { name: 'Flux REG-CANCELLED' })).toHaveCount(0);
  await page.getByRole('button', { name: 'Extinde REG-CANCELLED' }).click();
  await expect(page.getByText('Înregistrare duplicată')).toBeVisible();
});

test('registratura opens PDF export with a usable default interval', async ({ page }) => {
  await authenticatedRegistratura(page);
  await page.getByRole('button', { name: 'Exportă registrul în PDF' }).click();
  const dialog = page.getByRole('dialog', { name: 'Selectați intervalul de date pentru export PDF' });
  await expect(dialog.getByLabel('Data început')).not.toHaveValue('');
  await expect(dialog.getByLabel('Data sfârșit')).not.toHaveValue('');
  await expect(dialog.getByRole('button', { name: 'Generează PDF' })).toBeEnabled();
});
