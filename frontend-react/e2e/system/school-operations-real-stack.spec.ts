...
async function clickOpenPopoverAction(page: Page, name: string): Promise<void> {
  ...
  // to the Menu popup that is still marked open so this remains a real
  // interaction with the current row menu rather than a fading prior overlay.
  const menu = page.locator('[data-scope="menu"][data-part="popup"][data-open] [role="menu"]').last();
...
test('React creates portfolio relations and drives transfer/valorification lifecycle; DB verifies contracts', async ({ page }) => {
  ...
  sql(`set app.actor_subject='e2e';
    ...
    update portfolio_custody_upload_intents set status='committed',final_disposition='teacher_access',recovery_committed_at=now() where id='${institutionalCustodyIntentID}'`);
