...
test('personnel and evaluation children, decisions and compliance are persisted through React', async ({ page }) => {
  ...
  await createRelated(page, 'Adaugă încadrări', `/api/education/personnel/records/${personnel.id}/assignments`, { 'Titlu încadrare': 'Profesor', Tip: 'titular', Stare: 'active', 'Atribuit la': '2026-09-01', 'Ore săptămânale': '18' });
  await page.getByRole('button', { name: 'Documente dosar' }).click(); await createRelated(page, 'Adaugă documente dosar', `/api/education/personnel/records/${personnel.id}/file-documents`, { 'Categorie document': 'contract', Titlu: 'Contract', 'Nivel confidențialitate': 'restricted', 'Domeniu fișier': 'personnel', 'Emis la': '2026-09-01' });
  await page.getByRole('button', { name: 'Cazuri disciplinare' }).click(); await createRelated(page, 'Adaugă cazuri disciplinare', `/api/education/personnel/records/${personnel.id}/disciplinary-cases`, { 'Tip caz': 'review', Stare: 'open', 'Raportat la': '2026-09-01' });
  await page.getByRole('button', { name: 'Evenimente acces' }).click(); await createRelated(page, 'Adaugă evenimente acces', `/api/education/personnel/records/${personnel.id}/access-events`, { Tip: 'consultare', 'Accesat la': '2026-09-01', Operator: 'Director', 'Rol operator': 'director', 'Canal acces': 'web', Scop: 'verificare' });
  ...
  await createRelated(page, 'Adaugă autoevaluări', `/api/education/evaluations/records/${evaluation.id}/self-reviews`, { 'Finalizat la': '2026-09-01', 'Tip relatare': 'annual', Secțiune: 'Predare', Stare: 'draft' });
  await page.getByRole('button', { name: 'Criterii' }).click(); await createRelated(page, 'Adaugă criterii', `/api/education/evaluations/records/${evaluation.id}/criteria`, { Categorie: 'teaching', Criteriu: 'Calitate', Maxim: '100', Stare: 'draft' });
  ...
  await page.getByRole('button', { name: 'Comunicări rezultat' }).click(); await createRelated(page, 'Adaugă comunicări rezultat', `/api/education/evaluations/records/${evaluation.id}/result-issues`, { 'Tip document': 'result', Destinatar: personnelName, Canal: 'email', 'Stare livrare': 'sent', 'Emis la': '2026-09-01' });
