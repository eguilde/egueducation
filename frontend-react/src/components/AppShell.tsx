...
function NavigationContent({ tenantTitle, onNavigate }: { tenantTitle: string; onNavigate?: () => void }) {
    ...
    const { ready: policyReady, canPolicy } = useInstitutionPolicy();
    ...
    const items = useMemo(() => user && authorizationReady ? navigation.filter((item) => {
        ...
        return (!item.delegatedEducation || hasActiveDelegatedEducationGrant(educationGrants)) && (!item.permission || permits(item.permission)) && (!item.permissions || item.permissions.some(permits)) && (!item.module || session?.modules.some((module) => module.code === item.module && module.active)) && (!item.policyCapability || (policyReady && canPolicy(item.policyCapability)));
    }) : [], [authorizationReady, canEducation, canPolicy, educationGrants, has, policyReady, session?.modules, user]);
