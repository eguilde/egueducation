...
export interface ArchiveDocumentVersion { id: string; document_id: string; version_no: number; source_sha256: string; source_size_bytes: number; page_count: number; text_status: string; created_at: string; chunk_count?: number }
...
export interface ArchiveSearch { q?: string; mode?: SearchMode; status?: string; source_kind?: string; taxonomy_code?: string; document_date_from?: string; document_date_to?: string; received_at_from?: string; received_at_to?: string; page?: string; pageSize?: string; sort?: string; direction?: string }
...
export interface ArchiveJob { id: string; document_id?: string; version_id?: string; job_type?: string; stage?: string; status: string; attempts: number; has_error?: boolean; error_summary?: string; created_at?: string; available_at?: string; updated_at?: string; started_at?: string; finished_at?: string }
export interface ArchiveAdminHealth { storage_enabled: boolean; storage: { status: string; message?: string }; ocr: { configured: boolean; provider: string; status: string; message?: string }; queue: { pending: number; running: number; succeeded: number; failed: number } }
export interface ArchiveAdminStats { documents_by_status: Record<string, number>; jobs_by_status: Record<string, number>; total_documents: number; total_bytes: number; total_pages: number; total_jobs: number; queued: number; processing: number; completed: number; failed: number }
export type ArchiveClassificationReviewFilterState = 'pending_review' | 'needs_review';
export type ArchiveClassificationReviewState = ArchiveClassificationReviewFilterState | 'approved' | 'corrected';
export interface ArchiveClassificationField { value?: string; source: string; confidence: number; evidence?: string }
export interface ArchiveClassificationSuggestion { category: ArchiveClassificationField; fond: ArchiveClassificationField; series: ArchiveClassificationField; document_type: ArchiveClassificationField; document_date: ArchiveClassificationField; document_number: ArchiveClassificationField }
