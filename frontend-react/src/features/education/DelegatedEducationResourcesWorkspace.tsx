...
function personnelUpdateBody(detail: Record<string, unknown>, values: EditValues): components["schemas"]["CreatePersonnelRecordRequest"] { return { app_user_id: optional(detail, "app_user_id"), assigned_unit: values.assigned_unit.trim() || undefined, email: values.email.trim() || undefined, employment_type: patched(detail, values, "employment_type"), evaluation_status: patched(detail, values, "evaluation_status"), full_name: patched(detail, values, "full_name"), has_portfolio: booleanValue(detail, "has_portfolio"), mobility_stage: patched(detail, values, "mobility_stage"), notes: values.notes.trim() || undefined, phone: values.phone.trim() || undefined, role_title: patched(detail, values, "role_title"), school_year: required(detail, "school_year"), status: patched(detail, values, "status") }; }
