...
describe("PortfolioProcedureManager", () => {
  ...
  it("updates the separately labelled lifecycle value after procedure transitions", async () => { const client=api();client.transition=vi.fn().mockResolvedValueOnce({ ...procedure, status: "approved", updated_at: "2026-09-10T10:00:00Z" }).mockResolvedValueOnce({ ...procedure, status: "published", updated_at: "2026-09-10T10:01:00Z" });view(client);fireEvent.click(await screen.findByRole("button",{name:`Detalii și reguli pentru ${procedure.title}`}));const dialog=await screen.findByRole("dialog",{name:procedure.title});expect(within(dialog).getByLabelText("Stare procedură")).toHaveTextContent("draft");fireEvent.click(within(dialog).getByRole("button",{name:"Aprobă"}));fireEvent.change(within(dialog).getAllByRole("textbox").at(-1)!,{target:{value:"dovada aprobării"}});fireEvent.click(within(dialog).getAllByRole("button",{name:"Aprobă"}).at(-1)!);await waitFor(()=>expect(within(dialog).getByLabelText("Stare procedură")).toHaveTextContent("approved"));fireEvent.click(within(dialog).getByRole("button",{name:"Publică"}));fireEvent.change(within(dialog).getAllByRole("textbox").at(-1)!,{target:{value:"dovada publicării"}});fireEvent.click(within(dialog).getAllByRole("button",{name:"Publică"}).at(-1)!);await waitFor(()=>expect(within(dialog).getByLabelText("Stare procedură")).toHaveTextContent("published")); });
