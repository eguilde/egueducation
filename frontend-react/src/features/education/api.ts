...
const portfolioDocumentSorts = ["section_code", "component_code", "document_title", "description", "school_year", "subject_discipline", "applicable_class", "source_scope", "evidence_type", "issued_on", "chronological_index", "sensitive_data", "authenticity_status", "archive_version_no"] as const;
const portfolioDocumentSort = (sort?: string): typeof portfolioDocumentSorts[number] | undefined => portfolioDocumentSorts.includes(sort as typeof portfolioDocumentSorts[number]) ? sort as typeof portfolioDocumentSorts[number] : undefined;
const portfolioDocumentParams = (input: PortfolioDocumentListQuery = {}) => ({ page: input.page ?? 1, pageSize: input.pageSize ?? 50, sort: portfolioDocumentSort(input.sort), direction: input.direction, "filter.section_code": input.sectionCode, "filter.component_code": input.componentCode, "filter.document_title": input.documentTitle, "filter.description": input.description, "filter.school_year": input.schoolYear, "filter.subject_discipline": input.subjectDiscipline, "filter.applicable_class": input.applicableClass, "filter.competencies": input.competencies, "filter.source_scope": input.sourceScope, "filter.evidence_type": input.evidenceType, "filter.issued_on": input.issuedOn, "filter.chronological_index": input.chronologicalIndex, "filter.sensitive_data": input.sensitiveData, "filter.authenticity_status": input.authenticityStatus, "filter.archive_version_no": input.archiveVersionNo, "filter.archive_sha256": input.archiveSHA256 } as const);
const portfolioDocumentVersionParams = (input: PortfolioDocumentVersionListQuery = {}) => ({ page: input.page ?? 1, pageSize: input.pageSize ?? 20, sort: input.sort ?? "version_no", direction: input.direction ?? "desc", "filter.version_no": input.versionNo, "filter.change_type": input.changeType, "filter.changed_by": input.changedBy, "filter.changed_at": input.changedAt, "filter.reason": input.reason } as const);
const portfolioChecklistParams = (input: PortfolioChecklistListQuery = {}) => ({ page: input.page ?? 1, pageSize: input.pageSize ?? 50, sort: input.sort === "requirement_code" ? "requirement_code" : input.sort === "requirement_label" ? "requirement_label" : input.sort === "section_code" ? "section_code" : input.sort === "status" ? "status" : input.sort === "document_count" ? "document_count" : undefined, direction: input.direction, "filter.requirement_code": input.requirementCode, "filter.section_code": input.sectionCode, "filter.status": input.status } as const);
const portfolioOpisParams = (input: PortfolioOpisListQuery = {}) => ({ page: input.page ?? 1, pageSize: input.pageSize ?? 50, sort: input.sort === "section_code" ? "section_code" : input.sort === "component_code" ? "component_code" : input.sort === "entry_title" ? "entry_title" : input.sort === "chronological_index" ? "chronological_index" : input.sort === "document_reference" ? "document_reference" : undefined, direction: input.direction, "filter.section_code": input.sectionCode, "filter.component_code": input.componentCode, "filter.entry_title": input.entryTitle, "filter.chronological_index": input.chronologicalIndex, "filter.document_reference": input.documentReference } as const);
const portfolioCustodyParams = (input: PortfolioCustodyListQuery = {}) => ({ page: input.page ?? 1, pageSize: input.pageSize ?? 50, sort: input.sort === "event_type" ? "event_type" : input.sort === "holder_name" ? "holder_name" : input.sort === "holder_role" ? "holder_role" : input.sort === "started_on" ? "started_on" : input.sort === "ended_on" ? "ended_on" : undefined, direction: input.direction, "filter.event_type": input.eventType, "filter.holder_name": input.holderName, "filter.holder_role": input.holderRole, "filter.started_on": input.startedOn, "filter.ended_on": input.endedOn } as const);
const portfolioReviewParams = (input: PortfolioReviewListQuery = {}) => ({ page: input.page ?? 1, pageSize: input.pageSize ?? 50, sort: input.sort === "review_code" ? "review_code" : input.sort === "review_stage" ? "review_stage" : input.sort === "outcome" ? "outcome" : input.sort === "reviewer_name" ? "reviewer_name" : input.sort === "reviewed_on" ? "reviewed_on" : undefined, direction: input.direction, "filter.review_code": input.reviewCode, "filter.review_stage": input.reviewStage, "filter.outcome": input.outcome, "filter.reviewer_name": input.reviewerName } as const);
const portfolioTransferParams = (input: PortfolioTransferHistoryQuery = {}) => ({ ...portfolioPageParams(input), "filter.transfer_code": input.transferCode, "filter.transfer_type": input.transferType, "filter.destination_institution": input.destinationInstitution, "filter.status": input.status, "filter.handover_on": input.handoverOn });
const portfolioValorificationParams = (input: PortfolioValorificationListQuery = {}) => ({ ...portfolioPageParams(input), "filter.valorification_code": input.valorificationCode, "filter.scope": input.scope, "filter.status": input.status, "filter.target_institution": input.targetInstitution, "filter.started_on": input.startedOn });
...
export function createEducationApi(fetcher: AuthenticatedFetcher, apiBase = "/api"): EducationApi {
 ...
 return {
  ...
  async portfolioDocumentVersions(recordID, documentID, input = {}) { return unwrap(client.GET("/api/education/portfolios/records/{recordID}/documents/{documentID}/versions", { params: { path: { recordID, documentID }, query: portfolioDocumentVersionParams(input) } })); },
  ...
  async ownPortfolioRelated(id, resource, input = {}) {
    ...
    switch (resource) {
      case "documents": {
        const documentQuery = portfolioDocumentParams({ page: input.page, pageSize: input.pageSize, sort: input.sort, direction: input.direction, sectionCode: input.filters?.section_code, componentCode: input.filters?.component_code, documentTitle: input.filters?.document_title, description: input.filters?.description, schoolYear: input.filters?.school_year, subjectDiscipline: input.filters?.subject_discipline, applicableClass: input.filters?.applicable_class, competencies: input.filters?.competencies, sourceScope: input.filters?.source_scope, evidenceType: input.filters?.evidence_type, issuedOn: input.filters?.issued_on, chronologicalIndex: input.filters?.chronological_index, sensitiveData: input.filters?.sensitive_data, authenticityStatus: input.filters?.authenticity_status, archiveVersionNo: input.filters?.archive_version_no, archiveSHA256: input.filters?.archive_sha256 });
        return unwrap(client.GET("/api/education/portfolios/me/{recordID}/documents", { params: { path: { recordID: id }, query: documentQuery } }));
      }
