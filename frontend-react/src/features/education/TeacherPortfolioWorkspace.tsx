  ...
  return <Card.Root><Card.Body><Card.Title>{relatedLabels[resource]}</Card.Title><Card.Content><div className="flex flex-col gap-2">
    ...
    <DataTable.Root data={data.items as Record<string, unknown>[]} dataKey="id" scrollable className="max-h-[min(28rem,55dvh)] min-h-64 overflow-auto">
      <DataTable.Table>
        ...
        <DataTable.TBody>{({ item, index }) => { const record = item as EducationRecord; return <DataTable.Row key={record.id} index={index}>{columns.map((column) => <DataTable.Cell key={column.field}>{column.field === "status" || column.field === "authenticity_status" || column.field === "outcome" ? <Tag value={column.value(record) || "—"} severity={column.value(record) === "validated" || column.value(record) === "complete" ? "success" : "secondary"} /> : column.value(record) || "—"}</DataTable.Cell>)}<DataTable.Cell frozen alignFrozen="right">{resource === "documents" && canDelete ? <Button iconOnly rounded size="small" variant="outlined" severity="danger" aria-label={`Șterge ${String(record.document_title ?? "document")}`} title="Șterge" onClick={() => onDelete?.(record.id)}><i className="pi pi-trash" aria-hidden="true" /></Button> : <span>—</span>}</DataTable.Cell></DataTable.Row>; }}</DataTable.TBody>
      </DataTable.Table>
    </DataTable.Root>
...
export function TeacherPortfolioWorkspace({ api, canManageOwn }: { api: EducationApi; canManageOwn: boolean }) {
  ...
  return <section aria-label="Portofoliul meu profesional" className="flex flex-col gap-4">
    ...
    {addingDocument && <PortfolioDocumentForm saving={saving} archiveDocuments={archiveDocuments} portfolioSections={portfolioSections} onCancel={() => setAddingDocument(false)} onSave={(value) => void addDocument(value)} />}
