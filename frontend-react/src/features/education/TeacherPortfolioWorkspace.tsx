...
import { AngleLeft, AngleRight, History, Pencil, Plus, Refresh, Send, Trash } from "@primeicons/react";
...
import type { EducationApi, EducationListQuery, EducationPage, EducationRecord, OwnPortfolio, OwnPortfolioArchiveDocument, OwnPortfolioDocumentInput, OwnPortfolioInput, PortfolioDeclarationEvidence, PortfolioDeclarationTemplate, PortfolioDocument, PortfolioReviewEvent, PortfolioSection } from "./types";
import { PortfolioDownloadPanel } from "./PortfolioDownloadPanel";
import { PortfolioAppliedProcedurePanel } from "./PortfolioAppliedProcedurePanel";
...
const submitErrorMessage = (error: unknown) => {
  const code = error instanceof Error ? typeof error.cause === "string" ? error.cause : error.message : "";
...
const documentInput = (record: EducationRecord, schoolYear: string): OwnPortfolioDocumentInput => ({
  section_code: String(record.section_code ?? ""), component_code: String(record.component_code ?? ""), document_title: String(record.document_title ?? ""), description: String(record.description ?? ""), school_year: String(record.school_year ?? schoolYear), subject_discipline: String(record.subject_discipline ?? ""), applicable_class: String(record.applicable_class ?? ""), competencies: Array.isArray(record.competencies) ? record.competencies.map(String) : [], evidence_type: String(record.evidence_type ?? ""), issued_on: String(record.issued_on ?? ""), added_on: String(record.added_on ?? ""), chronological_index: typeof record.chronological_index === "number" ? record.chronological_index : 1, sensitive_data: Boolean(record.sensitive_data), file_reference: String(record.file_reference ?? ""), notes: String(record.notes ?? ""),
});
  ...
  return <div className="sticky bottom-0 z-10 flex flex-wrap items-center justify-between gap-2" aria-label="Paginare">
    ...
    <div className="flex items-center gap-2">
      <Select.Root value={pageSize} options={[10, 20, 50, 100].map((value) => ({ label: String(value), value }))} optionLabel="label" optionValue="value" onValueChange={(event: SelectValueChangeEvent) => onChange(1, Number(event.value))}>
        ...
        <Select.Portal><Select.Positioner><Select.Popup><Select.List className="max-h-64 overflow-y-auto" /></Select.Popup></Select.Positioner></Select.Portal>
      </Select.Root>
      <Button iconOnly rounded variant="outlined" severity="secondary" aria-label="Pagina anterioară" disabled={disabled || page <= 1} onClick={() => onChange(page - 1, pageSize)}><AngleLeft aria-hidden="true" /></Button>
      <span>Pagina {page} din {lastPage}</span>
      <Button iconOnly rounded variant="outlined" severity="secondary" aria-label="Pagina următoare" disabled={disabled || page >= lastPage} onClick={() => onChange(page + 1, pageSize)}><AngleRight aria-hidden="true" /></Button>
    </div>
  </div>;
}

function RelatedRecords({ api, portfolioID, resource, canAdd, canEdit, canDelete, revision, onAdd, onEdit, onDelete }: { api: EducationApi; portfolioID: string; resource: RelatedResource; canAdd: boolean; canEdit: boolean; canDelete: boolean; revision: number; onAdd?: () => void; onEdit?: (record: EducationRecord) => void; onDelete?: (id: string) => void }) {
  ...
  const load = useCallback(async (next: EducationListQuery) => {
    ...
    try { const result = await api.ownPortfolioRelated(portfolioID, resource, next); setData(result); }
    ...
    <DataTable.Root data={data.items as Record<string, unknown>[]} dataKey="id" scrollable className="max-h-[min(28rem,55dvh)] min-h-64 overflow-auto">
      <DataTable.Table>
        <DataTable.THead className="sticky top-0 z-10"><DataTable.THeadRow>
          ...
          <DataTable.THeadCell frozen alignFrozen="right"><span className="flex items-center justify-between gap-2"><span>Acțiuni</span>{resource === "documents" && canAdd && <Button iconOnly rounded size="small" aria-label="Adaugă document" title="Adaugă document" onClick={onAdd}><Plus aria-hidden="true" /></Button>}</span></DataTable.THeadCell>
        </DataTable.THeadRow></DataTable.THead>
        <DataTable.TBody>{({ item, index }) => { const record = item as EducationRecord; return <DataTable.Row key={record.id} index={index}>{columns.map((column) => <DataTable.Cell key={column.field}>{column.field === "status" || column.field === "authenticity_status" || column.field === "outcome" ? <Tag severity={column.value(record) === "validated" || column.value(record) === "complete" ? "success" : "secondary"}>{column.value(record) || "—"}</Tag> : column.value(record) || "—"}</DataTable.Cell>)}<DataTable.Cell frozen alignFrozen="right">{resource === "documents" ? <div className="flex items-center gap-1"><Button iconOnly rounded size="small" variant="text" severity="secondary" aria-label={`Istoric versiuni ${String(record.document_title ?? "document")}`} title="Istoric versiuni" onClick={() => setVersionDocument({ id: record.id, document_title: String(record.document_title ?? "Document") })}><History aria-hidden="true" /></Button>{canEdit && <Button iconOnly rounded size="small" variant="text" aria-label={`Editează ${String(record.document_title ?? "document")}`} title="Editează" onClick={() => onEdit?.(record)}><Pencil aria-hidden="true" /></Button>}{canDelete && <Button iconOnly rounded size="small" variant="outlined" severity="danger" aria-label={`Șterge ${String(record.document_title ?? "document")}`} title="Șterge" onClick={() => onDelete?.(record.id)}><Trash aria-hidden="true" /></Button>}</div> : <span>—</span>}</DataTable.Cell></DataTable.Row>; }}</DataTable.TBody>
      </DataTable.Table>
    </DataTable.Root>
...
function PortfolioForm({ value, saving, onCancel, onSave }: {
  ...
  const [form, setForm] = useState<OwnPortfolioInput>(() => ({
...
function PortfolioDocumentForm({ api, portfolioID, onArchiveReady, saving, archiveDocuments, portfolioSections, schoolYear, onCancel, onSave }: { api: EducationApi; portfolioID: string; onArchiveReady: (items: OwnPortfolioArchiveDocument[]) => void; saving: boolean; archiveDocuments: readonly OwnPortfolioArchiveDocument[]; portfolioSections: readonly PortfolioSection[]; schoolYear: string; onCancel: () => void; onSave: (input: OwnPortfolioDocumentInput) => void }) {
...
export function TeacherPortfolioWorkspace({ api, canManageOwn, canExportOwn = false }: { api: EducationApi; canManageOwn: boolean; canExportOwn?: boolean }) {
  ...
  const [editingDocument, setEditingDocument] = useState<{ id: string; value: OwnPortfolioDocumentInput }>();
  ...
  const [reviewFeedback, setReviewFeedback] = useState<PortfolioReviewEvent[]>([]);
  ...
  useEffect(() => {
    ...
    void api.ownPortfolioRelated(selected.id, "reviews", { page: 1, pageSize: 1, sort: "reviewed_on", direction: "desc" }).then((result) => {
      if (active) setReviewFeedback(result.items.filter(isPortfolioReview));
    }).catch(() => { if (active) setReviewFeedback([]); });
    return () => { active = false; };
  }, [api, selected?.id]);
  useEffect(() => {
    let active = true;
  ...
  const save = async (input: OwnPortfolioInput) => {
    ...
    try {
      ...
      setError(error instanceof Error && error.cause === "education_portfolio_published_procedure_required"
        ? "Școala nu are încă o procedură de portofoliu publicată. Contactați directorul sau administratorul școlii pentru aprobarea și publicarea procedurii."
        : "Portofoliul nu a putut fi salvat. Verificați datele și drepturile active.");
    }
  ...
  const editDocument = (document: EducationRecord) => {
    if (!selected || !editable(selected) || !canManageOwn) return;
    setEditingDocument({ id: document.id, value: documentInput(document, selected.school_year) });
  };
  const updateDocument = async (input: OwnPortfolioDocumentInput) => {
    if (!selected || !editingDocument || !editable(selected) || !canManageOwn) return;
    setSaving(true); setError(undefined);
    try { await api.updateOwnPortfolioDocument(selected.id, editingDocument.id, input); setEditingDocument(undefined); setNotice("Metadatele documentului au fost actualizate."); setRelatedRevision((current) => current + 1); }
    catch { setError("Documentul nu a putut fi actualizat. Verificați metadatele și drepturile active."); }
    finally { setSaving(false); }
  };
  ...
  const latestReview = reviewFeedback.reduce<PortfolioReviewEvent | undefined>((latest, review) => !latest || review.reviewed_on > latest.reviewed_on ? review : latest, undefined);
  return <section aria-label="Portofoliul meu profesional" className="flex flex-col gap-4">
    ...
    {loading ? <div className="flex justify-center p-6" role="status"><ProgressSpinner.Root><ProgressSpinner.Range><ProgressSpinner.Track /><ProgressSpinner.Value /></ProgressSpinner.Range></ProgressSpinner.Root></div> : items.length === 0 ? <Card.Root><Card.Body><Card.Content><Message.Root severity="info"><Message.Content><Message.Text>Nu există încă un portofoliu profesional pentru contul dvs. {canManageOwn ? "Creați o ciornă pentru anul școlar curent." : "Contactați administratorul instituției dacă trebuie alocat un portofoliu."}</Message.Text></Message.Content></Message.Root></Card.Content></Card.Body></Card.Root> : <div className="grid gap-4 lg:grid-cols-[minmax(16rem,0.8fr)_minmax(0,1.5fr)]"><Card.Root><Card.Body><Card.Title>Portofoliile mele</Card.Title><Card.Content><div className="flex flex-col gap-2">{items.map((item) => <Button key={item.id} variant={selected?.id === item.id ? undefined : "outlined"} severity={selected?.id === item.id ? undefined : "secondary"} onClick={() => setSelected(item)}><span className="flex w-full items-center justify-between gap-2"><span>{item.school_year}</span><Tag severity={severity(item.status)}>{statusLabel(item.status)}</Tag></span></Button>)}</div></Card.Content></Card.Body></Card.Root>
      {selected && <Card.Root><Card.Body><Card.Title>{selected.portfolio_code}</Card.Title><Card.Content><div className="flex flex-col gap-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><strong>{selected.owner_name}</strong><p>{selected.owner_role} · An școlar {selected.school_year}</p></div><Tag severity={severity(selected.status)}>{statusLabel(selected.status)}</Tag></div><div className="grid gap-3 xl:grid-cols-3"><Card.Root><Card.Body><Card.Title>Stare de completare</Card.Title><Card.Content><ul className="flex list-none flex-col gap-2 p-0 m-0">{checklist.map(([label, value, complete]) => <li key={label} className="flex items-center justify-between gap-2"><span>{label}</span><Tag severity={complete ? "success" : "warn"}>{value}</Tag></li>)}</ul></Card.Content></Card.Body></Card.Root><Card.Root><Card.Body><Card.Title>Flux și acțiunea următoare</Card.Title><Card.Content><p>{selected.status === "returned" ? "Consultați observațiile evaluatorului, completați corecțiile solicitate, apoi retrimiteți portofoliul." : editable(selected) ? "Completați checklistul, salvați ciorna și trimiteți portofoliul spre verificare." : selected.status === "submitted" ? "Portofoliul este în verificare instituțională." : selected.status === "validated" ? "Portofoliul este validat; îl puteți consulta." : "Așteptați următoarea acțiune a instituției."}</p>{selected.status === "returned" && (latestReview ? <Message.Root severity="warn"><Message.Content><Message.Text><strong>Observații evaluator · {latestReview.reviewed_on}</strong><br />{latestReview.notes}<br />Rezultat: {latestReview.outcome} · Documente lipsă: {latestReview.missing_documents} · Scor conformitate: {latestReview.compliance_score}</Message.Text></Message.Content></Message.Root> : <Message.Root severity="warn"><Message.Content><Message.Text>Portofoliul a fost returnat, dar observațiile evaluatorului nu au putut fi încărcate. Reîncercați.</Message.Text></Message.Content></Message.Root>)}</Card.Content></Card.Body></Card.Root><Card.Root><Card.Body><Card.Title>Declarații obligatorii</Card.Title><Card.Content><div className="flex flex-col gap-2">{declarationsLoading ? <ProgressSpinner.Root><ProgressSpinner.Range><ProgressSpinner.Track /><ProgressSpinner.Value /></ProgressSpinner.Range></ProgressSpinner.Root> : declarations.templates.length === 0 ? <Message.Root severity="warn"><Message.Content><Message.Text>Declarațiile oficiale nu sunt disponibile.</Message.Text></Message.Content></Message.Root> : declarations.templates.map((template) => { const accepted = acknowledged(template); return <div key={`${template.declaration_type}:${template.declaration_version}`} className="flex flex-wrap items-center justify-between gap-2"><div><strong>{declarationLabel(template.declaration_type)}</strong><p>Versiunea {template.declaration_version}</p></div>{accepted ? <Tag severity="success">Confirmată</Tag> : <Button size="small" variant="outlined" disabled={!editable(selected) || !canManageOwn || saving} onClick={() => { setDeclarationTemplate(template); setDeclarationConfirmed(false); }}>Citește și confirmă</Button>}</div>; })}</div></Card.Content></Card.Body></Card.Root></div><div className="grid gap-3 xl:grid-cols-2"><RelatedRecords api={api} portfolioID={selected.id} resource="documents" canAdd={Boolean(editable(selected) && canManageOwn && !saving)} canEdit={Boolean(editable(selected) && canManageOwn && !saving)} canDelete={Boolean(editable(selected) && canManageOwn && !saving)} revision={relatedRevision} onAdd={() => setAddingDocument(true)} onEdit={editDocument} onDelete={(id) => setConfirmation({ kind: "delete", documentID: id })} /><RelatedRecords api={api} portfolioID={selected.id} resource="checklist" canAdd={false} canEdit={false} canDelete={false} revision={relatedRevision} /><RelatedRecords api={api} portfolioID={selected.id} resource="opis" canAdd={false} canEdit={false} canDelete={false} revision={relatedRevision} /><RelatedRecords api={api} portfolioID={selected.id} resource="reviews" canAdd={false} canEdit={false} canDelete={false} revision={relatedRevision} /></div>{editable(selected) && canManageOwn && <div className="flex flex-wrap gap-2"><Button size="small" variant="outlined" disabled={saving} onClick={() => void regenerateOpis()}><Refresh aria-hidden="true" /> Regenerare opis</Button><Button variant="outlined" severity="secondary" disabled={saving} onClick={() => void openEdit(selected)}><Pencil aria-hidden="true" /> Editează ciorna</Button><Button disabled={saving || !declarationsComplete} onClick={() => setConfirmation({ kind: "submit" })}><Send aria-hidden="true" /> Trimite spre verificare</Button></div>}</div></Card.Content></Card.Body></Card.Root>}</div>}
    {selected && <PortfolioAppliedProcedurePanel key={`procedure:${selected.id}`} portfolioID={selected.id} disabled={loading || saving} load={api.ownPortfolioAppliedProcedure} />}
    {selected && <Button variant="outlined" disabled={loading} onClick={() => setLifecycleHistoryID(selected.id)}>Istoric operații de protecție</Button>}
    {lifecycleHistoryID && selected?.id === lifecycleHistoryID && <PortfolioLifecycleHistory api={api} portfolioID={lifecycleHistoryID} canRetry={false} canSubmitDisposition onClose={() => setLifecycleHistoryID(undefined)} />}
    {selected && <PortfolioDownloadPanel key={`download:${selected.id}`} portfolioID={selected.id} portfolioCode={selected.portfolio_code} download={api.exportOwnPortfolio} canExport={canExportOwn} disabled={loading || saving} />}
...
{addingDocument && selected && <PortfolioDocumentForm api={api} portfolioID={selected.id} saving={saving} archiveDocuments={archiveDocuments} portfolioSections={portfolioSections} schoolYear={selected.school_year} onArchiveReady={(items) => setArchiveDocuments((current) => [...current.filter((document) => !items.some((item) => item.id === document.id)), ...items])} onCancel={() => setAddingDocument(false)} onSave={(value) => void addDocument(value)} />}
    {editingDocument && <PortfolioDocumentEditForm value={editingDocument.value} saving={saving} archiveDocuments={archiveDocuments} portfolioSections={portfolioSections} onCancel={() => setEditingDocument(undefined)} onSave={(value) => void updateDocument(value)} />}
...
function isPortfolioReview(value: EducationRecord): value is PortfolioReviewEvent {
  return typeof value.reviewed_on === "string" && typeof value.notes === "string" && typeof value.outcome === "string" && typeof value.missing_documents === "number" && typeof value.compliance_score === "number";
}
