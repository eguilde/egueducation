...
function SignatureEvidenceActions({ evidence, canValidate, onDetail, onRevalidate }: { evidence: SignatureEvidence; canValidate: boolean; onDetail(evidence: SignatureEvidence): void; onRevalidate(evidence: SignatureEvidence): void }) {
  const [open, setOpen] = useState(false);
  const select = (action: () => void) => {
    setOpen(false);
    window.requestAnimationFrame(action);
  };
  return <Popover.Root open={open} onOpenChange={(event: { value?: boolean }) => setOpen(Boolean(event.value))}><Popover.Trigger as={Button} iconOnly rounded size="small" variant="text" aria-label="Acțiuni dovadă" title="Acțiuni dovadă"><i className="pi pi-ellipsis-v" aria-hidden="true" /></Popover.Trigger><Popover.Portal><Popover.Positioner side="left"><Popover.Popup><Popover.Content><div className="flex flex-col gap-1 p-1" role="menu"><Button size="small" variant="text" onClick={() => select(() => onDetail(evidence))}><i className="pi pi-eye" aria-hidden="true" />Detalii</Button>{canValidate && <Button size="small" variant="text" onClick={() => select(() => onRevalidate(evidence))}><i className="pi pi-verified" aria-hidden="true" />Revalidează</Button>}</div></Popover.Content></Popover.Popup></Popover.Positioner></Popover.Portal></Popover.Root>;
}
