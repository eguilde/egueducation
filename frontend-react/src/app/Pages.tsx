...
import { useInstitutionPolicy } from '../features/institution/InstitutionPolicyProvider';
...
export function RequirePermission({ permission, module, policyCapability, children }: { permission: string; module?: string; policyCapability?: string; children: React.ReactNode }) { const { ready, authorizationReady, has, canEducation, session } = useAuth(); const { ready: policyReady, canPolicy } = useInstitutionPolicy(); if (!ready || (module === 'education' && !authorizationReady) || (policyCapability && !policyReady)) return <div className="flex justify-center p-8"><Spinner /></div>; const moduleActive = !module || Boolean(session?.modules.some((item) => item.code === module && item.active)); const allowed = module === 'education' ? canEducation(permission) : has(permission); const policyAllowed = !policyCapability || canPolicy(policyCapability); return allowed && moduleActive && policyAllowed ? <>{children}</> : <Navigate to="/" replace />; }
