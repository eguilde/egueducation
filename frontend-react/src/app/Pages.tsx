...
export function RequirePermission({ permission, module, children }: { permission: string; module?: string; children: React.ReactNode }) { const { ready, authorizationReady, has, canEducation, session } = useAuth(); if (!ready || (module === 'education' && !authorizationReady)) return <div className="flex justify-center p-8"><Spinner /></div>; const moduleActive = !module || Boolean(session?.modules.some((item) => item.code === module && item.active)); const allowed = module === 'education' ? canEducation(permission) : has(permission); return allowed && moduleActive ? <>{children}</> : <Navigate to="/" replace />; }
