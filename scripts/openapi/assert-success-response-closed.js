...
function walk(schema, where) {
  ...
  if (schema.type === 'object' && !schema['x-free-form-property'] && !schema.properties && (schema.additionalProperties === undefined || schema.additionalProperties === true)) failures.push(`${where} (open object)`);
  for (const part of [...(schema.allOf||[]), ...(schema.oneOf||[]), ...(schema.anyOf||[])]) walk(part, where);
  if (schema.items) walk(schema.items, where);
  if (schema.additionalProperties && typeof schema.additionalProperties === 'object') walk(schema.additionalProperties, where);
